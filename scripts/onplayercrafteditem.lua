...
function main(event,noCraftExp)
	...
	elseif event.itemstack.name == "cursed-arrow" then
		local player = game.getplayer(event.playerindex)
		remover.item = event.itemstack
		remover.player = player.name
		if player.character then
			...
			if arrows > 0 then
				if player.character.health > 25 then
					local stats = glob.cursed[player.name].stats
					if player.caninsert{name="cursed-arrow-" .. stats.range.level, count = 1} then
						local lasthp = glob.cursed[player.name].aux.lasthp
						...
						glob.cursed[player.name].aux.lasthp = lasthp
						player.insert({name="cursed-arrow-"..stats.range.level,count=event.itemstack.count})
						local cant = math.floor((stats.explore.level * datos.resExplore) / 100)
						if math.random(100 / datos.resExplore) <= stats.explore.level - (cant * (100 / datos.resExplore)) then
							cant = cant + 1
						end
						if cant > 0 then
							player.insert{name="cursed-arrow-"..stats.range.level,count=(event.itemstack.count * cant)}
							if glob.cursed[player.name].opt[4] == true then
								player.print({"msg.cursed",{"msg.item-bonus",event.itemstack.count * cant , game.getlocaliseditemname("cursed-arrow-" .. stats.range.level)}})
							end
							game.createentity({name="flying-text", position=player.position, color = player.color, text={"msg.item-bonus-flying",event.itemstack.count * cant , game.getlocaliseditemname("cursed-arrow-" .. stats.range.level)} })
						end
