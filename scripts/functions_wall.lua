...
function addWall(player,num,position,name,invip)
	local walls = glob.cursed[player.name].walls
	local ret
	if invip ~= nil then
		if invip == "yp" or invip == "yn" then
			ret = game.createentity{name="cursed-wall-i1010",position=position, force=player.force}
		elseif invip == "xp" or invip == "xn" then
			ret = game.createentity{name="cursed-wall-i0101",position=position, force=player.force}
		end
		walls[num].invi[invip].destructible = false
		walls[num].invi[invip].active = false
	end
	...
	for n = 1, 4 do
		for i = 1,#all["pipes" .. n] do
			if string.sub(all["pipes" .. n][i].name,1,12) == "cursed-wall-" then
				...
				if invip ~= nil and (invip == "xp" or invip == "xn") and (n == 4 or n == 3) then
					walls[num].invi[invip].active = true
				elseif invip ~= nil and (invip == "yp" or invip == "yn") and (n == 2 or n == 1) then
					walls[num].invi[invip].active = true
				end
			end
		end
	...
	if invip == nil then
		...
		ret = game.createentity{name=nexpipename,position=position, force=player.force} -- walls[num].sides["wall" .. side][#walls[num].sides["wall" .. side] + 1] = game.createentity{name=nexpipename,position=position, force=player.force}
	end
	maxFluidStorage(player.name,num)
	return ret
end
...
function range(level,upgrade)
	return math.floor(level * 0.25 + upgrade * 0.125 + 16)
end